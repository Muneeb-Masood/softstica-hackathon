...
## 2026-08-08 — Capped-segment explanation name-mismatch: verified fixed

Real-world testing on the time slider flagged a risk: `generate_timeline_explanations` (Phase 9) caps real Gemini calls at `EXPLANATION_SEGMENT_CAP=6` per timeline query; any #1-pick segment beyond the cap needs *some* text next to its ranked card. If that text were a reused explanation from an earlier segment, it would name a *different* AED than the one actually shown as #1 for that hour -- the card and the prose would contradict each other.

Inspected the current `backend/explanation.py` implementation: capped segments already set `top_explanation`/`runnerup_explanation` to `None` (nothing that could name the wrong AED) and instead attach a fixed, generic `_CAPPED_NOTE` string that names no AED at all ("Ranking changed again in this window... only the narrative explanation text is unavailable here; open a fresh query..."). No code path reuses a prior segment's named explanation text for a capped hour. This matches option (a) from the fix request (generic note, no AED name) exactly, and the frontend (`ResultsPanel.jsx`) only renders `explanation` text when non-null, so a capped hour shows the generic note only -- never a mismatched name.

No source change was needed. Restarted the backend fresh (guaranteeing the running process reflects this exact code, not a possibly-stale in-memory version from before this logic was written) and re-verified two ways:

1. **Synthetic 8-changepoint test** (`backend/test_timeline_explanations.py`, committed): builds a synthetic 24-hour timeline using 9 distinct real Sentosa AED records as the #1 pick, changepoints at hours 1-8 (9 segments, 8 changepoints -- real Sentosa data never naturally produces this many in one day per CLAUDE.md section 10, hence "synthetic"). Result: `{'segments': 9, 'gemini_calls'/'cache_hits' summing to 6, 'capped_segments': 3}` -- hours 0-5 get real explanation text (verified each mentions its own true #1 AED), hours 6-23 (3 capped segments) all have `top_explanation=None`, `runnerup_explanation=None`, and a note that was checked against every candidate AED's building name and AED_ID -- zero matches. Test passed with 0 failures.
2. **Live endpoint sanity check**: `POST /rank/timeline` against a real query (1.25653, 103.81503, 2026-08-08) returns `{'segments': 3, 'capped_segments': 0}` -- confirms real Sentosa data doesn't exercise the cap at all (consistent with the AEDs-are-mostly-24/7 assumption), which is exactly why the synthetic test above is the only way to exercise this path.

Conclusion: the capped-segment name-mismatch failure mode does not occur in the current code. Kept `test_timeline_explanations.py` in the repo (not a scratch script this time) so this can be re-run directly in future sessions without reconstructing it.

## 2026-08-09 — Entrance augmentation (Option A): before/after numbers

Added `backend/build_building_footprints.py` + `backend/entrance_augmentation.py` to address the brief's "entrances" criterion directly. OSM's pedestrian network only maps public streets, not building interiors, so points near/inside a building previously snapped to whichever single nearest street node happened to be closest (the "KNOWN LIMITATION" documented in `distance_ranking.py` and `crowd_simulation.py`, and CLAUDE.md section 8 Phase 5's cited "~51.5% of Sentosa AEDs share a nearest-node" finding). The fix samples several evenly-spaced points around the perimeter of every OSM building footprint that has an AED within 25m of it, and adds each as a synthetic graph node connected to the nearest real street node -- giving those AEDs several plausible "doors" to snap to instead of one. Tagged `synthetic=True` end to end; a walking result whose shortest path uses one is flagged `uses_synthetic_entrance=True` (same honesty convention as `uses_stairs`/`crosses_unmarked_road`), and surfaced as a user-facing note in `main.py`'s `_mobility_warning`.

**Scale:** 109 of 494 downloaded building footprints were within 25m of at least one AED. First pass used `MAX_ENTRANCE_POINTS=10`, which added 792 synthetic nodes but under-served large single-footprint buildings (a 10-point cap on a ~500m-perimeter building works out to one entrance every ~50m). Raised to `MAX_ENTRANCE_POINTS=24` so `perimeter/TARGET_SPACING_M` (15m target spacing) sets the actual count for most buildings instead of hitting the cap; final pass added **1134 synthetic nodes** (graph grew from 657/1778 nodes/edges to 1791/4046).

**Dataset-wide snap-collision rate** (`distance_ranking.compute_snap_collision_confidence`'s underlying node-sharing, all 66 Sentosa AEDs): **51.5% → 15.2%** of AEDs sharing a nearest-node with at least one other AED (10 of 66 still share, all in buildings with no matched footprint or where the footprint's own geometry still puts two AEDs close to the same perimeter sample point). Unchanged between the 10-point and 24-point cap -- this metric is dominated by buildings that already had few enough AEDs to fully de-collide at 10 points.

**Universal Studios Singapore**, the brief's own example attraction and `test_crowd_simulation.py`'s test case (mapped as many separate smaller OSM building footprints, so raising the per-building cap barely mattered here):
- AED-level: 19 AEDs previously collapsed onto 10 distinct nearest-graph-nodes → now **19 AEDs / 19 distinct nodes** (every USS AED now has `distance_confidence=1.0`, no snap-collision penalty).
- Crowd-simulation grid level (64-point 8x8 grid over USS's AED bbox): 17 distinct snap nodes (`avg_points_per_snap_node`≈3.8, `indoor_snap_degenerate=True`) → **57 distinct nodes at the 10-point cap, 58 at the 24-point cap** (avg 1.1 points/node, `indoor_snap_degenerate=False` either way).

**Equarius Hotel** (one single ~504m-perimeter building, all its AEDs on one footprint -- the harder case, and the one that motivated raising the cap): 7 distinct snap nodes pre-augmentation → **12 at the 10-point cap → 16 at the 24-point cap**, out of 64 grid points. Still `indoor_snap_degenerate=True` at 16/64 (needs >=32 to clear). This is a real, honest limit of the approach for one large building with AEDs spread across it: more perimeter points have diminishing returns once interior grid points are already roughly equidistant to several of them, and going further would mean building-shape-aware sampling (e.g. weighting points toward the building's long sides), not just more points -- out of scope for this pass. The flag correctly keeps reporting this case as data-coverage-limited rather than hiding it.

`test_crowd_simulation.py`'s USS assertions were updated to match this (was asserting the old degenerate behavior as the expected/correct outcome, which is now backwards -- the fix is the point of the change, so the test should assert the improvement, not the flaw).

**What this is not:** not a real indoor floor plan, not a verified entrance location -- the synthetic points are evenly-spaced guesses around a building's OSM-mapped outline, not surveyed doors. Buildings with no mapped OSM footprint, or with no AED nearby, are untouched and still hit the original limitation. This narrows the coverage gap; it does not close it, and Equarius Hotel shows that narrowing is not the same as resolving.
